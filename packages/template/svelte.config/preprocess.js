...
export const preprocessConfig = [
  preprocess({
    ...
    replace: [
      ...
      [/(?<!\w)\$images/gm, () => '/' + images_path.relativeTo(workspace_path).setSeparator('/').fullName],
      [/(?<!\w)\$pages/gm, () => '/' + pages_path.relativeTo(workspace_path).setSeparator('/').fullName]
    ],
    pug: {
  ...
  preprocess.replace([[
    /^\s+import\s+Layout_MDSVEX_DEFAULT, \* as Components from '([^']+)'/gmi,
    (match, layoutPath) => {
